...
/*
 ...
 3. create an oracle with the identical name, ie. KMD and format must start with IhhL (height, blockhash, merkleroot, deposit balance)
...
CScript EncodeGatewaysBindOpRet(uint8_t funcid,uint8_t version, CPubKey mypk, uint256 tokenid,std::string coin,int64_t totalsupply,uint256 oracletxid,uint8_t M,uint8_t N,std::vector<CPubKey> gatewaypubkeys,uint8_t taddr,uint8_t prefix,uint8_t prefix2,uint8_t wiftype)
{
    ...
    vopret = E_MARSHAL(ss << evalcode << funcid << version << coin << totalsupply << oracletxid << M << N << gatewaypubkeys << taddr << prefix << prefix2 << wiftype);
    return(EncodeTokenOpRetV1(tokenid,pubkeys, { vopret }));
}

uint8_t DecodeGatewaysBindOpRet(char *depositaddr,const CScript &scriptPubKey,uint8_t &version,CPubKey &mypk,uint256 &tokenid,std::string &coin,int64_t &totalsupply,uint256 &oracletxid,uint8_t &M,uint8_t &N,std::vector<CPubKey> &gatewaypubkeys,uint8_t &taddr,uint8_t &prefix,uint8_t &prefix2,uint8_t &wiftype)
{
...
CScript EncodeGatewaysDepositOpRet(uint8_t funcid,uint8_t version,CPubKey mypk,uint256 tokenid,uint256 bindtxid,std::string refcoin,std::vector<CPubKey> publishers,std::vector<uint256>txids,int32_t height,uint256 cointxid,int32_t claimvout,std::string deposithex,std::vector<uint8_t>proof,CPubKey destpub,int64_t amount)
{
    vscript_t vopret; uint8_t evalcode = EVAL_GATEWAYS; std::vector<CPubKey> pubkeys;

    pubkeys.push_back(destpub);
    vopret = E_MARSHAL(ss << evalcode << funcid << version << tokenid << bindtxid << refcoin << publishers << txids << height << cointxid << claimvout << deposithex << proof << destpub << amount);
    return(EncodeTokenOpRetV1(tokenid,pubkeys, { vopret }));
}

uint8_t DecodeGatewaysDepositOpRet(const CScript &scriptPubKey,uint8_t &version,CPubKey &mypk,uint256 &tokenid,uint256 &bindtxid,std::string &refcoin,std::vector<CPubKey>&publishers,std::vector<uint256>&txids,int32_t &height,uint256 &cointxid, int32_t &claimvout,std::string &deposithex,std::vector<uint8_t> &proof,CPubKey &destpub,int64_t &amount)
{
    std::vector<uint8_t> vopret,vOpretExtra; uint8_t *script,e,f; std::vector<CPubKey> pubkeys;
    std::vector<vscript_t>  oprets;
    ...
    script = (uint8_t *)vopret.data();
    if ( vopret.size() > 2 && script[0]==EVAL_GATEWAYS && E_UNMARSHAL(vopret,ss >> e; ss >> f; ss >> version; ss >> tokenid; ss >> bindtxid; ss >> refcoin; ss >> publishers; ss >> txids; ss >> height; ss >> cointxid; ss >> claimvout; ss >> deposithex; ss >> proof; ss >> destpub; ss >> amount) != 0 )
    {
        return(f);
    }
    return(0);
...
CScript EncodeGatewaysWithdrawOpRet(uint8_t funcid,uint8_t version,CPubKey mypk,uint256 tokenid,uint256 bindtxid,std::string refcoin,CPubKey withdrawpub,int64_t amount)
{
    uint8_t evalcode = EVAL_GATEWAYS; struct CCcontract_info *cp,C; CPubKey gatewayspk;
    std::vector<CPubKey> pubkeys;
    vscript_t vopret;
    ...
    gatewayspk = GetUnspendable(cp,0);
    pubkeys.push_back(gatewayspk);
    vopret = E_MARSHAL(ss << evalcode << funcid << version << mypk << bindtxid << refcoin << withdrawpub << amount);        
    return(EncodeTokenOpRetV1(tokenid,pubkeys, { vopret }));
}

uint8_t DecodeGatewaysWithdrawOpRet(const CScript &scriptPubKey,uint8_t &version,CPubKey &mypk,uint256 &tokenid,uint256 &bindtxid,std::string &refcoin,CPubKey &withdrawpub,int64_t &amount)
{
    std::vector<vscript_t>  oprets;
    std::vector<uint8_t> vopret,vOpretExtra; uint8_t *script,e,f; std::vector<CPubKey> pubkeys;
    ...
    script = (uint8_t *)vopret.data();
    if ( vopret.size() > 2 && script[0]==EVAL_GATEWAYS && E_UNMARSHAL(vopret, ss >> e; ss >> f; ss >> version; ss >> mypk; ss >> bindtxid; ss >> refcoin; ss >> withdrawpub; ss >> amount) != 0 )
    {
        return(f);
    }
    return(0);
...
CScript EncodeGatewaysWithdrawSignOpRet(uint8_t funcid,uint8_t version,CPubKey mypk,uint256 withdrawtxid, uint256 lasttxid,std::vector<CPubKey> signingpubkeys,std::string refcoin,uint8_t K,std::string hex)
{
    CScript opret; uint8_t evalcode = EVAL_GATEWAYS;
    opret << OP_RETURN << E_MARSHAL(ss << evalcode << funcid << version << mypk << withdrawtxid << lasttxid << signingpubkeys << refcoin << K << hex);        
    return(opret);
}

uint8_t DecodeGatewaysWithdrawSignOpRet(const CScript &scriptPubKey,uint8_t &version,CPubKey &mypk,uint256 &withdrawtxid, uint256 &lasttxid,std::vector<CPubKey> &signingpubkeys,std::string &refcoin,uint8_t &K,std::string &hex)
{
    std::vector<uint8_t> vopret; uint8_t *script,e,f;

    GetOpReturnData(scriptPubKey, vopret);
    script = (uint8_t *)vopret.data();
    if ( vopret.size() > 2 && script[0]==EVAL_GATEWAYS && E_UNMARSHAL(vopret,ss >> e; ss >> f; ss >> version; ss >> mypk; ss >> withdrawtxid; ss >> lasttxid; ss >> signingpubkeys; ss >> refcoin; ss >> K; ss >> hex) != 0 )
    {
        return(f);
    }
    return(0);
...
CScript EncodeGatewaysMarkDoneOpRet(uint8_t funcid,uint8_t version,CPubKey mypk,uint256 withdrawtxid,std::string refcoin,uint256 withdrawsigntxid)
{
    CScript opret; uint8_t evalcode = EVAL_GATEWAYS;

    opret << OP_RETURN << E_MARSHAL(ss << evalcode << funcid << version << mypk << withdrawtxid << refcoin << withdrawsigntxid);        
    return(opret);
}

uint8_t DecodeGatewaysMarkDoneOpRet(const CScript &scriptPubKey,uint8_t &version,CPubKey &mypk,uint256 &withdrawtxid,std::string &refcoin, uint256 &withdrawsigntxid)
{
    std::vector<uint8_t> vopret; uint8_t *script,e,f;

    GetOpReturnData(scriptPubKey, vopret);
    script = (uint8_t *)vopret.data();
    if ( vopret.size() > 2 && script[0]==EVAL_GATEWAYS && E_UNMARSHAL(vopret,ss >> e; ss >> f; ss >> version; ss >> mypk; ss >> withdrawtxid; ss >> refcoin; ss >> withdrawsigntxid;) != 0 )
    {
        return(f);
    }
    return(0);
...
uint8_t DecodeGatewaysOpRet(const CScript &scriptPubKey,uint8_t &version)
{
    std::vector<vscript_t>  oprets;
...
{
    int32_t numvouts,claimvout,height; int64_t amount; std::string coin,deposithex; std::vector<CPubKey> publishers;
    std::vector<uint256>txids; uint256 tokenid,bindtxid,cointxid; std::vector<uint8_t> proof; CPubKey pk,claimpubkey; uint8_t version;

    if ( (numvouts= tx.vout.size()) > 0 )
    {
        if ( DecodeGatewaysDepositOpRet(tx.vout[numvouts-1].scriptPubKey,version,pk,tokenid,bindtxid,coin,publishers,txids,height,cointxid,claimvout,deposithex,proof,claimpubkey,amount) == 'D' && claimpubkey == mypk )
        {
            return(amount);
        }
    }
...
int32_t GatewaysBindExists(struct CCcontract_info *cp,CPubKey gatewayspk,uint256 reftokenid)
{
    ...
    std::vector<uint256> txids; CPubKey mypk;
    ...
    {
        if ( myGetTransaction(*it,tx,hashBlock) != 0 && (numvouts= tx.vout.size()) > 0 && DecodeGatewaysOpRet(tx.vout[numvouts-1].scriptPubKey,version)=='B' )
        {
            if ( DecodeGatewaysBindOpRet(depositaddr,tx.vout[numvouts-1].scriptPubKey,version,mypk,tokenid,coin,totalsupply,oracletxid,M,N,pubkeys,taddr,prefix,prefix2,wiftype) == 'B' )
            {
    ...
    for (std::vector<CTransaction>::const_iterator it=tmp_txs.begin(); it!=tmp_txs.end(); it++)
    {
        ...
        if ((numvouts=txmempool.vout.size()) > 0 && txmempool.vout[0].nValue==CC_MARKER_VALUE && DecodeGatewaysOpRet(tx.vout[numvouts-1].scriptPubKey,version)=='B')
            if (DecodeGatewaysBindOpRet(depositaddr,tx.vout[numvouts-1].scriptPubKey,version,mypk,tokenid,coin,totalsupply,oracletxid,M,N,pubkeys,taddr,prefix,prefix2,wiftype) == 'B' &&
            tokenid == reftokenid)
                return(1);
    }
...
bool GatewaysExactAmounts(Eval* eval,const CTransaction &tx)
{
    CTransaction vinTx; uint256 hashBlock; int32_t i,numvouts; int64_t inputs=0,outputs=0; uint8_t version,funcid;

    if ((numvouts=tx.vout.size()) > 0 && (funcid=DecodeGatewaysOpRet(tx.vout[numvouts-1].scriptPubKey,version))!=0)
    {        
        switch (funcid)
        {
            case 'B':
...
bool ValidateGatewaysBindTx(Eval* eval,const CTransaction& bindtx)
{
    int32_t i,numvouts; uint8_t version,funcid,M,N,taddr,prefix,prefix2,wiftype; struct CCcontract_info *cp,C; 
    CTransaction prevTx; uint256 hashblock,tokenid,tmptokenid,oracletxid; int64_t totalsupply,fullsupply;
    std::vector<CPubKey> pubkeys; std::vector<vscript_t> oprets; CPubKey mypk,gatewayspk;
    char gatewaystokensaddr[64],gatewaysmarkeraddr[64],depositaddr[64]; std::string refcoin,name,description,format;

    cp = CCinit(&C,EVAL_TOKENS);
    gatewayspk = GetUnspendable(cp,0);      
    GetTokensCCaddress(cp, gatewaystokensaddr, gatewayspk);
    GetCCaddress(cp, gatewaysmarkeraddr, gatewayspk);
    CCOpretCheck(eval,bindtx,true,true,true);
    ExactAmounts(eval,bindtx,ASSETCHAINS_CCZEROTXFEE[EVAL_GATEWAYS]?0:CC_TXFEE);
    i=0;
    while (i<bindtx.vin.size() && (cp->ismyvin)(bindtx.vin[i].scriptSig) != 0)
    {  
        if (myGetTransaction(bindtx.vin[i].prevout.hash,prevTx,hashblock)!= 0 && (numvouts=prevTx.vout.size()) > 0 
            && (funcid=DecodeTokenOpRetV1(prevTx.vout[numvouts-1].scriptPubKey, tmptokenid, pubkeys, oprets))!=0 
         && ((funcid=='c' && prevTx.GetHash()==tokenid) || (funcid!='c' && tmptokenid==tokenid)))
            i++;
        else break;
    }
    while (i<bindtx.vin.size() && IsCCInput(bindtx.vin[i].scriptSig) == 0) i++;
    if (i!=bindtx.vin.size())
        return eval->Invalid("invalid vin structure for gatewaysbind!");
    else if ((numvouts=bindtx.vout.size()) < 1 || DecodeGatewaysBindOpRet(depositaddr,bindtx.vout[numvouts-1].scriptPubKey,version,mypk,tokenid,refcoin,totalsupply,oracletxid,M,N,pubkeys,taddr,prefix,prefix2,wiftype) != 'B')
        return eval->Invalid("invalid gatewaysbind OP_RETURN data!"); 
    else if (bindtx.vout.size()!=103)
        ...
        return eval->Invalid("not enough pubkeys("+std::to_string(pubkeys.size())+" for N."+std::to_string(N)+" gatewaysbind ");
    else if ( (fullsupply=CCfullsupply(tokenid)) != totalsupply )
        return eval->Invalid("Gateway bind."+refcoin+" ("+tokenid.GetHex()+") globaladdr."+cp->unspendableCCaddr+" totalsupply "+std::to_string((double)totalsupply/COIN)+" != fullsupply "+std::to_string((double)totalsupply/COIN));
    else if (myGetTransaction(oracletxid,prevTx,hashblock) == 0 || (numvouts=prevTx.vout.size()) <= 0 )
        return eval->Invalid("cant find oracletxid "+oracletxid.GetHex());
    else if ( DecodeOraclesCreateOpRet(prevTx.vout[numvouts-1].scriptPubKey,name,description,format) != 'C' )
    ...
    else if ( ConstrainVout(bindtx.vout[100],1,gatewaysmarkeraddr,CC_MARKER_VALUE)==0 )
        return eval->Invalid("vout.0 is CC for gatewaysbind or invalid amount!");
    for (int i=0;i<100;i++)
        if ( ConstrainVout(bindtx.vout[i],1,gatewaystokensaddr,totalsupply/100)==0 ) 
...
{
    CTransaction prevTx; uint256 hashblock,tmp_txid; int32_t i,numvouts;
    uint8_t version; char funcid,tmpaddr[64];

    if ((*cp->ismyvin)(tx.vin[index].scriptSig) == 0)
        return eval->Invalid("vin."+std::to_string(index)+" is gatewaysCC for gateways tx!");
    ...
    else if ((numvouts=prevTx.vout.size()) > 0 && DecodeGatewaysOpRet(prevTx.vout[numvouts-1].scriptPubKey,version) == 0) 
    ...
    else if (prevTx.GetHash()!=bindtxid && bindtxid!=tmp_txid) 
        return eval->Invalid("invalid vin."+std::to_string(index)+" tx, different bind txid!");
    if (funcid='W' || funcid=='B') return (true);
    while (i<prevTx.vin.size())
    {
        if ((*cp->ismyvin)(prevTx.vin[i].scriptSig) != 0) return (true);
        i++;
    }
    return eval->Invalid("invalid vin."+std::to_string(index)+" tx, not a valid gatewaysCC tx!");
}

int32_t ValidateGatewaysTokensInputs(struct CCcontract_info *cp,Eval* eval, const CTransaction& tx,int32_t index, uint256 bindtxid)
{
    char tokensaddr[64]; int32_t i=0;

    GetTokensCCaddress(cp,tokensaddr,GetUnspendable(cp,0));
    while (i<tx.vin.size() && (cp->ismyvin)(tx.vin[i].scriptSig) != 0)
    {
        if (ValidateGatewaysVin(cp,eval,tx,i,bindtxid,tokensaddr,0) != 0) i++;
        else return 0;
    }
    return (i);
}

int32_t ValidateTokensInputs(Eval* eval, const CTransaction& tx,int32_t index, uint256 bindtxid)
{
    char tokensaddr[64]; int32_t i=0; struct CCcontract_info *cp,C; uint256 hasblock; CTransaction prevtx;

    cp=CCinit(&C,EVAL_TOKENS);
    while (i<tx.vin.size() && (cp->ismyvin)(tx.vin[i].scriptSig) != 0)
    {
        if (myGetTransaction(tx.vin[i].prevout.hash,prevtx,hasblock)!=0 && TokensValidate(cp,eval,prevtx,0) != 0) i++;
        else return 0;
    }
    return (i);
}
...
{
    int32_t numvins,numvouts,preventCCvins,preventCCvouts,i,height,claimvout; bool retval; uint8_t version,tmpv,funcid,K,tmpK,M,N,taddr,prefix,prefix2,wiftype;
    char destaddr[65],fromaddr[64],depositaddr[65],gatewaystokensaddr[65]; int64_t totalsupply,amount;
    std::vector<uint256> txids; std::vector<CPubKey> pubkeys,publishers,signingpubkeys,tmpsigningpubkeys; std::vector<uint8_t> proof;  
    uint256 hashblock,txid,bindtxid,deposittxid,withdrawtxid,tmpwithdrawtxid,withdrawsigntxid,lasttxid,tmplasttxid,tokenid,tmptokenid,oracletxid,cointxid,merkleroot,mhash;
    CTransaction tmptx; std::string refcoin,tmprefcoin,hex,tmphex,name,description,format; CPubKey tmppk,mypk,gatewayspk,destpub;
    ...
    {      
        ...
        gatewayspk = GetUnspendable(cp,0);      
        GetTokensCCaddress(cp, gatewaystokensaddr, gatewayspk);      
        if ( (funcid = DecodeGatewaysOpRet(tx.vout[numvouts-1].scriptPubKey,version)) != 0)
        {
            switch ( funcid )
            {
                case 'B':
                    ...
                    break;
                case 'D':
                    //vin.0: CC gateways tokens input
                    //...
                    //vin.m: normal inputs
                    //...
                    ...
                    else if (DecodeGatewaysDepositOpRet(tx.vout[numvouts-1].scriptPubKey,version,mypk,tmptokenid,bindtxid,tmprefcoin,publishers,txids,height,cointxid,claimvout,hex,proof,destpub,amount) != 'D')
                    ...
                    else if ( (i=ValidateGatewaysTokensInputs(cp,eval,tx,0,bindtxid)) == 0 )
                        return (false);
                    else if ( Getscriptaddress(fromaddr,CScript() << ParseHex(HexStr(mypk)) << OP_CHECKSIG)==0 || ValidateNormalVins(eval,tx,i,fromaddr) == 0 )
                        return (false);
                    else if (_GetCCaddress(destaddr,EVAL_TOKENS,destpub)==0 || ConstrainVout(tx.vout[0],1,destaddr,amount)==0)
                    ...
                    else if ( ConstrainVout(tx.vout[2],1,gatewaystokensaddr,0)==0 )
                        return eval->Invalid("invalid vout.2 tokens change to gateways global address for gatewaysdeposit!");
                    else if ( ConstrainVout(tx.vout[3],0,fromaddr,0)==0 )
                        return eval->Invalid("vout.3 is normal change for gatewaysdeposit!");
                    else if (!destpub.IsFullyValid())
                        return eval->Invalid("invalid deposit tx destination pubkey for gatewaysdeposit");
                    else if (myGetTransaction(bindtxid,tmptx,hashblock) == 0)
                        return eval->Invalid("invalid gatewaysbind txid!");
                    else if (ValidateGatewaysBindTx(eval,tmptx)==0)
                        return (false);
                    else if (DecodeGatewaysBindOpRet(depositaddr,tmptx.vout[numvouts-1].scriptPubKey,tmpv,tmppk,tokenid,refcoin,totalsupply,oracletxid,M,N,pubkeys,taddr,prefix,prefix2,wiftype) != 'B')
                        return eval->Invalid("invalid gatewaysbind OP_RETURN data!"); 
                    else if (tmprefcoin!=refcoin)
                        return eval->Invalid("refcoin different than in bind tx for gatewasydeposit");
                    else if (tmptokenid!=tokenid)
                        return eval->Invalid("tokenid different than in bind tx for gatewasydeposit");
                    else if (amount>totalsupply)
                        return eval->Invalid("deposit amount greater then bind total supply for gatewasydeposit");                        
                    ...
                    else 
                    {
                        ...
                        if ( merkleroot == zeroid || m < N/2 )
                            return eval->Invalid("couldnt find merkleroot for ht."+std::to_string(height)+" " +tmprefcoin+" oracle."+oracletxid.GetHex()+" m."+std::to_string(m)+" vs n."+std::to_string(N/2)+" needed");
                        else if (GatewaysVerify(depositaddr,oracletxid,claimvout,tmprefcoin,cointxid,hex,proof,merkleroot,destpub,taddr,prefix,prefix2)!=amount)
                ...
                case 'W':
                    ...
                    //vin.1: normal input
                    //vout.0: CC vout of gateways tokens back to gateways tokens CC address                  
                    //vout.1: CC vout marker to gateways CC address                
                    //vout.2: CC vout change of tokens back to owners pubkey
                    //vout.3: normal change                                          
                    //vout.4: opreturn - 'W' tokenid bindtxid refcoin withdrawpub amount
                ...
                case 'S':          
                    ...
                    //vout.2: opreturn - 'S' withdrawtxid lasttxid mypk refcoin number_of_signs hex
                    if (numvouts!=3)
                        return eval->Invalid("invalid number of vouts for gatewayswithdrawsign tx!");
                    if (DecodeGatewaysWithdrawSignOpRet(tx.vout[numvouts-1].scriptPubKey,version,mypk,withdrawtxid,lasttxid,signingpubkeys,refcoin,K,hex)!='S')
                        return eval->Invalid("invalid gatewayswithdrawsign OP_RETURN data!");
                    if (tx.vin[0].prevout.hash!=lasttxid)
                        return eval->Invalid("invalid lasttxid and vin.0 txid not matching!");
                    else if (lasttxid==withdrawtxid)
                    {
                        if (myGetTransaction(withdrawtxid,tmptx,hashblock) == 0)
                            return eval->Invalid("invalid gatewayswithdraw txid!");
                        else if ((numvouts=tmptx.vout.size()) > 0 && DecodeGatewaysWithdrawOpRet(tmptx.vout[numvouts-1].scriptPubKey,tmpv,tmppk,tmptokenid,bindtxid,tmprefcoin,destpub,amount)!='W')
                            ...
                            return eval->Invalid("invalid deposit tx destination pubkey for gatewayswithdraw");
                        else if ( (i=ValidateTokensInputs(eval,tmptx,0,bindtxid)) == 0 )
                            return (false);
                        else if ( Getscriptaddress(fromaddr,CScript() << ParseHex(HexStr(tmppk)) << OP_CHECKSIG)==0 || ValidateNormalVins(eval,tmptx,i,fromaddr) == 0 )
                            return (false);
                        else if ( ConstrainVout(tmptx.vout[0],1,gatewaystokensaddr,amount)==0)
                            return eval->Invalid("invalid vout.0 tokens to gateways global address for gatewayswithdraw!");
                        else if ( ConstrainVout(tmptx.vout[1],1,cp->unspendableCCaddr,CC_MARKER_VALUE)==0)
                            return eval->Invalid("invalid vout.1 marker vout for gatewayswithdraw!");
                        else if ( _GetCCaddress(destaddr,EVAL_TOKENS,tmppk)==0 || ConstrainVout(tmptx.vout[2],1,destaddr,0)==0)
                            return eval->Invalid("invalid vout.2 tokens change back to tx creator address for gatewayswithdraw!");
                        else if ( ConstrainVout(tmptx.vout[3],0,fromaddr,0)==0 )
                            return eval->Invalid("invalid vout.3 is normal change for gatewayswithdraw!");
                    }
                    else if (lasttxid!=withdrawtxid)
                    {
                        if (myGetTransaction(lasttxid,tmptx,hashblock) == 0)
                            return eval->Invalid("invalid last txid!");
                        else if ((numvouts=tmptx.vout.size()) > 0 && DecodeGatewaysWithdrawSignOpRet(tmptx.vout[numvouts-1].scriptPubKey,tmpv,tmppk,tmpwithdrawtxid,tmplasttxid,tmpsigningpubkeys,tmprefcoin,tmpK,tmphex)!='S')
                            return eval->Invalid("invalid last gatewayswithdrawsign OP_RETURN data!");
                        else if (CompareHexTx(hex,tmphex)==0)
                            return eval->Invalid("invalid gatewayswithdrawsign, modifying initial tx vouts in hex!");
                        else if (tmpwithdrawtxid!=withdrawtxid)
                            return eval->Invalid("invalid this tx withdraw txid different than last gatewayswithdrawsign withdraw txid!");
                        else if (K<=tmpK)
                            return eval->Invalid("invalid number of signes must be greater then previous gatewayswithdrawsign tx!");
                    }
                    else if (ValidateGatewaysVin(cp,eval,tx,0,bindtxid,cp->unspendableCCaddr,CC_MARKER_VALUE)==0)
                        return (false);
                    else if (Getscriptaddress(fromaddr,CScript() << ParseHex(HexStr(mypk)) << OP_CHECKSIG)==0 || ValidateNormalVins(eval,tx,1,fromaddr) == 0)
                        return (false);
                    else if (ConstrainVout(tx.vout[0],1,cp->unspendableCCaddr,CC_MARKER_VALUE) == 0 )
                        return eval->Invalid("invalid vout.0 invalid marker for gatewayswithdrawsign!");
                    else if ( ConstrainVout(tx.vout[1],0,fromaddr,0)==0 )
                        return eval->Invalid("invalid vout.1 is normal change for gatewayswithdrawsign!");
                    else if (CheckVinPk(tx,1,pubkeys)==0)
                        return eval->Invalid("vin.1 invalid, gatewayswithdrawsign must be created by one of the gateways pubkeys owner!");
                ...
                case 'M':                        
                    ...
                    if (DecodeGatewaysMarkDoneOpRet(tx.vout[numvouts-1].scriptPubKey,version,mypk,withdrawtxid,refcoin,withdrawsigntxid)!='M')
                    ...
                    else if ((numvouts=tmptx.vout.size()) > 0 && DecodeGatewaysWithdrawSignOpRet(tmptx.vout[numvouts-1].scriptPubKey,tmpv,tmppk,withdrawtxid,tmplasttxid,signingpubkeys,tmprefcoin,K,hex)!='S')
                        return eval->Invalid("invalid gatewayswithdrawsign OP_RETURN data!");
                        ...
                        return eval->Invalid("invalid gatewaysbind txid!");
                    else if ((numvouts=tmptx.vout.size()) < 1 || DecodeGatewaysBindOpRet(depositaddr,tmptx.vout[numvouts-1].scriptPubKey,tmpv,tmppk,tokenid,tmprefcoin,totalsupply,oracletxid,M,N,pubkeys,taddr,prefix,prefix2,wiftype) != 'B')
                        return eval->Invalid("invalid gatewaysbind OP_RETURN data!"); 
                    else if (tmprefcoin!=refcoin)
...
{
    char coinaddr[64],depositaddr[64]; int64_t threshold,nValue,price,totalinputs = 0,totalsupply,amount; std::vector<uint256> txids; std::vector<uint8_t> proof; 
    CTransaction vintx,bindtx; int32_t vout,numvouts,n = 0,height,claimvout; uint8_t version,tmpv,M,N,evalcode,funcid,taddr,prefix,prefix2,wiftype; 
    std::vector<CPubKey> pubkeys,publishers; std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue> > unspentOutputs; 
    std::string refcoin,tmprefcoin,hex; CPubKey withdrawpub,destpub,tmppk,pubkey; uint256 tokenid,txid,oracletxid,tmpbindtxid,tmptokenid,deposittxid,hashBlock,cointxid; 

    if ( myGetTransaction(bindtxid,bindtx,hashBlock) != 0 )
    {
        if ((numvouts=bindtx.vout.size())!=0 && DecodeGatewaysBindOpRet(depositaddr,bindtx.vout[numvouts-1].scriptPubKey,version,tmppk,tokenid,refcoin,totalsupply,oracletxid,M,N,pubkeys,taddr,prefix,prefix2,wiftype) == 'B')
        {
            ...
            for (std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue> >::const_iterator it=unspentOutputs.begin(); it!=unspentOutputs.end(); it++)
            {
                ...
                {
                    funcid=DecodeGatewaysOpRet(vintx.vout[vintx.vout.size()-1].scriptPubKey,tmpv);
                    if (((funcid=='B' && bindtxid==txid) ||
                        (vout==2 && funcid=='D' && DecodeGatewaysDepositOpRet(vintx.vout[vintx.vout.size()-1].scriptPubKey,tmpv,tmppk,tmptokenid,tmpbindtxid,tmprefcoin,publishers,txids,height,cointxid,claimvout,hex,proof,pubkey,amount) == 'D' &&
                        tmpbindtxid==bindtxid && tmprefcoin==refcoin && tmptokenid==tokenid) ||
                        (vout==2 && funcid=='W' && DecodeGatewaysWithdrawOpRet(vintx.vout[vintx.vout.size()-1].scriptPubKey,tmpv,tmppk,tmptokenid,tmpbindtxid,tmprefcoin,withdrawpub,amount) == 'W' &&
...
UniValue GatewaysBind(const CPubKey& pk, uint64_t txfee,std::string coin,uint256 tokenid,int64_t totalsupply,uint256 oracletxid,uint8_t M,uint8_t N,std::vector<CPubKey> pubkeys,uint8_t p1,uint8_t p2,uint8_t p3,uint8_t p4)
{
    ...
    struct CCcontract_info *cp,*cpTokens,C,CTokens; std::string name,description,format; int32_t i,numvouts,n=0; int64_t fullsupply,tmp,datafee;
    ...
    if (pubkeys.size()!=n)
        CCERR_RESULT("importgateway",CCLOG_ERROR, stream << "different number of bind and oracle pubkeys " << n << "!=" << pubkeys.size() << std::endl);
    for (i=0; i<N; i++)
    {
    ...
    if ( (tmp=AddTokenCCInputs(cpTokens,mtx,mypk,tokenid,0,0)) != totalsupply )
        CCERR_RESULT("gatewayscc",CCLOG_ERROR, stream << "token balance on " << myTokenCCaddr << " " << (double)tmp/COIN << "!=" << (double)totalsupply/COIN);
    ...
    if ( name!=coin )
        CCERR_RESULT("gatewayscc",CCLOG_ERROR, stream << "mismatched oracle name "<<name<<" != " << coin);
    ...
    if (AddTokenCCInputs(cpTokens, mtx, mypk, tokenid, totalsupply, 64)==totalsupply)
    {
        if ( AddNormalinputs(mtx,mypk,txfee+CC_MARKER_VALUE,2,pk.IsValid()) >= txfee+CC_MARKER_VALUE )
        {
            ...
            return(FinalizeCCTxExt(pk.IsValid(),0,cp,mtx,mypk,txfee,EncodeGatewaysBindOpRet('B',GATEWAYSCC_VERSION,mypk,tokenid,coin,totalsupply,oracletxid,M,N,pubkeys,taddr,prefix,prefix2,wiftype)));
        }
        else CCERR_RESULT("gatewayscc",CCLOG_ERROR, stream << "cant find enough inputs");
    }
    else CCERR_RESULT("gatewayscc",CCLOG_ERROR, stream << "you must have total supply of tokens in your tokens address!");
}
...
{
    CMutableTransaction mtx = CreateNewContextualCMutableTransaction(Params().GetConsensus(), komodo_nextheight());
    CTransaction tx; CPubKey mypk,tmppk,gatewayspk; uint256 oracletxid,merkleroot,mhash,hashBlock,tokenid,txid;
    ...
    if ( DecodeGatewaysBindOpRet(depositaddr,tx.vout[numvouts-1].scriptPubKey,version,tmppk,tokenid,coin,totalsupply,oracletxid,M,N,pubkeys,taddr,prefix,prefix2,wiftype) != 'B' || refcoin != coin )
    ...
    if ((inputs=AddGatewaysInputs(cp, mtx, gatewayspk, bindtxid, amount, 60)) >= amount)
    {
        if ( AddNormalinputs(mtx,mypk,txfee+CC_MARKER_VALUE,3,pk.IsValid()) >= txfee+CC_MARKER_VALUE )
        {
            ...
            return(FinalizeCCTxExt(pk.IsValid(),0,cp,mtx,mypk,txfee,EncodeGatewaysDepositOpRet('D',GATEWAYSCC_VERSION,mypk,tokenid,bindtxid,coin,publishers,txids,height,cointxid,claimvout,deposithex,proof,destpub,amount)));
        }
        else CCERR_RESULT("gatewayscc",CCLOG_ERROR, stream << "cant find enough inputs");
    }
    else CCERR_RESULT("gatewayscc",CCLOG_ERROR, stream << "cant find enough token inputs from gateway");
}
...
{
    CMutableTransaction mtx = CreateNewContextualCMutableTransaction(Params().GetConsensus(), komodo_nextheight());
    CTransaction tx; CPubKey mypk,tmppk,gatewayspk,signerpk,tmpwithdrawpub; uint256 txid,tokenid,hashBlock,oracletxid,tmptokenid,tmpbindtxid,withdrawtxid; int32_t vout,numvouts,n,i;
    int64_t nValue,totalsupply,inputs,CCchange=0,tmpamount,balance; uint8_t version,funcid,K,M,N,taddr,prefix,prefix2,wiftype; std::string coin,hex;
    ...
    n = (int32_t)pubkeys.size();
    for (i=0; i<n; i++)
        if ( (balance=CCOraclesGetDepositBalance("gatewayscc-2",oracletxid,OraclesBatontxid(oracletxid,pubkeys[i])))==0 || amount > balance )
    ...
    if ((inputs = AddTokenCCInputs(cpTokens, mtx, mypk, tokenid, amount, 60)) >= amount)
    {
        if( AddNormalinputs(mtx, mypk, txfee+CC_MARKER_VALUE, 2,pk.IsValid()) >= txfee+CC_MARKER_VALUE )
        {
            ...
            return(FinalizeCCTxExt(pk.IsValid(),0, cpTokens, mtx, mypk, txfee,EncodeGatewaysWithdrawOpRet('W',GATEWAYSCC_VERSION,mypk,tokenid,bindtxid,refcoin,withdrawpub,amount)));
        }
        else CCERR_RESULT("gatewayscc",CCLOG_ERROR, stream << "cant find enough normal inputs");
    }
...
UniValue GatewaysWithdrawSign(const CPubKey& pk, uint64_t txfee,uint256 lasttxid,std::string refcoin, std::string hex)
{
    ...
    uint8_t version,tmpv,K=0,M,N,taddr,prefix,prefix2,wiftype; std::vector<CPubKey> pubkeys,signingpubkeys; int64_t totalsupply;
    ...
    if (myGetTransaction(lasttxid,tx,hashBlock)==0 || (numvouts= tx.vout.size())<=0
        || (funcid=DecodeGatewaysOpRet(tx.vout[numvouts-1].scriptPubKey,version))==0 || (funcid!='W' && funcid!='S'))
        CCERR_RESULT("gatewayscc",CCLOG_ERROR, stream << "invalid last txid" << lasttxid.GetHex());
    if (funcid=='W')
    {
        withdrawtxid=lasttxid;
        if (DecodeGatewaysWithdrawOpRet(tx.vout[numvouts-1].scriptPubKey,version,tmppk,tmptokenid,bindtxid,coin,withdrawpub,amount)!='W' || refcoin!=coin)
           CCERR_RESULT("gatewayscc",CCLOG_ERROR, stream << "cannot decode withdraw tx opret " << lasttxid.GetHex());
            ...
            CCERR_RESULT("gatewayscc",CCLOG_ERROR, stream << "can't find bind tx " << bindtxid.GetHex());
        else if (DecodeGatewaysBindOpRet(depositaddr,tmptx.vout[numvouts-1].scriptPubKey,tmpv,tmppk,tokenid,coin,totalsupply,oracletxid,M,N,pubkeys,taddr,prefix,prefix2,wiftype) != 'B' || refcoin!=coin || tokenid!=tmptokenid)
            CCERR_RESULT("gatewayscc",CCLOG_ERROR, stream << "invalid bind tx "<<bindtxid.GetHex());
    }
    else if (funcid=='S')
    {
        if (DecodeGatewaysWithdrawSignOpRet(tx.vout[numvouts-1].scriptPubKey,version,tmppk,withdrawtxid,tmplasttxid,signingpubkeys,coin,K,tmphex)!='S' || refcoin!=coin)
        ...
        else if (DecodeGatewaysWithdrawOpRet(tmptx.vout[numvouts-1].scriptPubKey,tmpv,tmppk,tmptokenid,bindtxid,coin,withdrawpub,amount)!='W' || refcoin!=coin)
    ...
    if (AddNormalinputs(mtx,mypk,txfee,3,pk.IsValid())>=txfee) 
    {
        ...
        return(FinalizeCCTxExt(pk.IsValid(),0,cp,mtx,mypk,txfee,EncodeGatewaysWithdrawSignOpRet('S',GATEWAYSCC_VERSION,mypk,withdrawtxid,lasttxid,signingpubkeys,refcoin,K+1,hex)));
    }
    else CCERR_RESULT("gatewayscc",CCLOG_ERROR, stream << "error adding funds for withdrawsign");
}
...
UniValue GatewaysMarkDone(const CPubKey& pk, uint64_t txfee,uint256 withdrawsigntxid,std::string refcoin)
{
    ...
    uint8_t version,tmpv,K,M,N,taddr,prefix,prefix2,wiftype; std::vector<CPubKey> pubkeys,signingpubkeys; int64_t amount,totalsupply; CPubKey withdrawpub;
    ...
    if (myGetTransaction(withdrawsigntxid,tx,hashBlock)==0 || (numvouts= tx.vout.size())<=0)
    ...
    else if (DecodeGatewaysWithdrawSignOpRet(tx.vout[numvouts-1].scriptPubKey,version,tmppk,withdrawtxid,tmplasttxid,signingpubkeys,coin,K,hex)!='S' || refcoin!=coin)
        ...
        CCERR_RESULT("gatewayscc",CCLOG_ERROR, stream << "invalid withdraw txid " << withdrawtxid.GetHex());
    else if (DecodeGatewaysWithdrawOpRet(tx.vout[numvouts-1].scriptPubKey,tmpv,tmppk,tmptokenid,bindtxid,coin,withdrawpub,amount)!='W' || refcoin!=coin)
        CCERR_RESULT("gatewayscc",CCLOG_ERROR, stream << "cannot decode withdraw tx opret " << withdrawtxid.GetHex());
    else if (myGetTransaction(bindtxid,tx,hashBlock)==0 || (numvouts=tx.vout.size())<=0)
        CCERR_RESULT("gatewayscc",CCLOG_ERROR, stream << "can't find bind tx " << bindtxid.GetHex());
    else if (DecodeGatewaysBindOpRet(depositaddr,tx.vout[numvouts-1].scriptPubKey,tmpv,tmppk,tokenid,coin,totalsupply,oracletxid,M,N,pubkeys,taddr,prefix,prefix2,wiftype) != 'B' || refcoin!=coin || tokenid!=tmptokenid)
            CCERR_RESULT("gatewayscc",CCLOG_ERROR, stream << "invalid bind tx "<< bindtxid.GetHex());
    ...
    if (AddNormalinputs(mtx,mypk,txfee,3)>=txfee) 
    {
        ...
        return(FinalizeCCTxExt(pk.IsValid(),0,cp,mtx,mypk,txfee,EncodeGatewaysMarkDoneOpRet('M',GATEWAYSCC_VERSION,mypk,withdrawtxid,refcoin,withdrawsigntxid)));
    }
    else CCERR_RESULT("gatewayscc",CCLOG_ERROR, stream << "error adding funds for markdone");
}
...
UniValue GatewaysPendingDeposits(const CPubKey& pk, uint256 bindtxid,std::string refcoin)
{
    ...
    CPubKey mypk,tmppk,gatewayspk,destpub; std::vector<CPubKey> pubkeys,publishers; std::vector<uint256> txids;
    uint256 tmpbindtxid,hashBlock,txid,tokenid,oracletxid,cointxid; uint8_t version,tmpv,M,N,taddr,prefix,prefix2,wiftype;
    ...
    for (std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue> >::const_iterator it=unspentOutputs.begin(); it!=unspentOutputs.end(); it++)
    {
        ...
        if ( vout == 0 && nValue == CC_MARKER_VALUE && myGetTransaction(txid,tx,hashBlock) != 0 && (numvouts=tx.vout.size())>0 &&
            DecodeGatewaysDepositOpRet(tx.vout[numvouts-1].scriptPubKey,tmpv,tmppk,tokenid,tmpbindtxid,coin,publishers,txids,height,cointxid,claimvout,hex,proof,destpub,amount) == 'D'
...
UniValue GatewaysPendingSignWithdraws(const CPubKey& pk, uint256 bindtxid,std::string refcoin)
{
    ...
    std::vector<CPubKey> msigpubkeys; uint256 hashBlock,txid,tmpbindtxid,tokenid,tmptokenid,oracletxid,withdrawtxid,tmplasttxid; uint8_t version,tmpv,K=0,M,N,taddr,prefix,prefix2,wiftype;
    ...
    if ( myGetTransaction(bindtxid,tx,hashBlock) == 0 || (numvouts= tx.vout.size()) <= 0 )
        CCERR_RESULT("gatewayscc",CCLOG_ERROR, stream << "cant find bindtxid " << bindtxid.GetHex());
    if ( DecodeGatewaysBindOpRet(depositaddr,tx.vout[numvouts-1].scriptPubKey,version,tmppk,tokenid,coin,totalsupply,oracletxid,M,N,msigpubkeys,taddr,prefix,prefix2,wiftype) != 'B' || refcoin != coin )
    ...
    myGet_mempool_txs(tmp_txs,EVAL_GATEWAYS,0);
    for (std::vector<CTransaction>::const_iterator it=tmp_txs.begin(); it!=tmp_txs.end(); it++)
    {
        tx = *it;
        vout=0;
        if (myIsutxo_spentinmempool(ignoretxid,ignorevin,tx.GetHash(),vout)==0 && (numvouts= tx.vout.size())>0 && tx.vout[vout].nValue == CC_MARKER_VALUE && 
            DecodeGatewaysWithdrawSignOpRet(tx.vout[numvouts-1].scriptPubKey,tmpv,tmppk,withdrawtxid,tmplasttxid,signingpubkeys,coin,K,hex)=='S' && myGetTransaction(withdrawtxid,withdrawtx,hashBlock)!=0
            && (numvouts=withdrawtx.vout.size())>0 && DecodeGatewaysWithdrawOpRet(withdrawtx.vout[numvouts-1].scriptPubKey,tmpv,tmppk,tmptokenid,tmpbindtxid,coin,withdrawpub,amount)=='W'
    ...
    if (txs.empty())
    {
        ...
        for (std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue> >::const_iterator it=unspentOutputs.begin(); it!=unspentOutputs.end(); it++)
        {
            ...
            if (myIsutxo_spentinmempool(ignoretxid,ignorevin,txid,vout) == 0 && vout == 0 && nValue == CC_MARKER_VALUE && myGetTransaction(txid,tx,hashBlock) != 0 &&
                (numvouts= tx.vout.size())>0 && DecodeGatewaysWithdrawSignOpRet(tx.vout[numvouts-1].scriptPubKey,tmpv,tmppk,withdrawtxid,tmplasttxid,signingpubkeys,coin,K,hex)=='S' && myGetTransaction(withdrawtxid,withdrawtx,hashBlock)!=0
                && (numvouts=withdrawtx.vout.size())>0 && DecodeGatewaysWithdrawOpRet(withdrawtx.vout[numvouts-1].scriptPubKey,tmpv,tmppk,tmptokenid,tmpbindtxid,coin,withdrawpub,amount)=='W' && refcoin==coin && tmpbindtxid==bindtxid && tmptokenid==tokenid)
            {
                txs.push_back(tx);
                break;
            } 
    ...
    if (txs.empty())
    {
        for (std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue> >::const_iterator it=unspentOutputs.begin(); it!=unspentOutputs.end(); it++)
        {
            ...
            nValue = (int64_t)it->second.satoshis;
            if (myIsutxo_spentinmempool(ignoretxid,ignorevin,txid,vout) == 0 && vout == 0 && nValue == CC_MARKER_VALUE && myGetTransaction(txid,tx,hashBlock) != 0 &&
                (numvouts= tx.vout.size())>0 && DecodeGatewaysWithdrawOpRet(tx.vout[numvouts-1].scriptPubKey,tmpv,tmppk,tmptokenid,tmpbindtxid,coin,withdrawpub,amount)=='W' &&
    ...
    for (std::vector<CTransaction>::const_iterator it=txs.begin(); it!=txs.end(); it++)
    {
        ...
        vout=0;
        K=0;
        if (myIsutxo_spentinmempool(ignoretxid,ignorevin,tx.GetHash(),vout) == 0 && tx.vout[vout].nValue == CC_MARKER_VALUE && (numvouts= tx.vout.size())>0 && (funcid=DecodeGatewaysOpRet(tx.vout[numvouts-1].scriptPubKey,tmpv))!=0 && (funcid=='W' || funcid=='S'))
        {
            txid=tx.GetHash();
            if (funcid=='W')
            {
                DecodeGatewaysWithdrawOpRet(tx.vout[numvouts-1].scriptPubKey,tmpv,tmppk,tmptokenid,tmpbindtxid,coin,withdrawpub,amount);
                withdrawtxid=txid;
            }
            else if (funcid=='S')
            {
                DecodeGatewaysWithdrawSignOpRet(tx.vout[numvouts-1].scriptPubKey,tmpv,tmppk,withdrawtxid,tmplasttxid,signingpubkeys,coin,K,hex);
                if (myGetTransaction(withdrawtxid,tx,hashBlock)==0 || (numvouts=tx.vout.size())<=0 || DecodeGatewaysWithdrawOpRet(tx.vout[numvouts-1].scriptPubKey,tmpv,tmppk,tmptokenid,tmpbindtxid,coin,withdrawpub,amount)!='W') continue;
            }
...
UniValue GatewaysSignedWithdraws(const CPubKey& pk, uint256 bindtxid,std::string refcoin)
{
    ...
    uint256 withdrawtxid,tmplasttxid,tmpbindtxid,tokenid,tmptokenid,hashBlock,txid,oracletxid; uint8_t version,tmpv,K,M,N,taddr,prefix,prefix2,wiftype;
    ...
    for (std::vector<CTransaction>::const_iterator it=tmp_txs.begin(); it!=tmp_txs.end(); it++)
    {
        ...
        vout=0;
        if (myIsutxo_spentinmempool(ignoretxid,ignorevin,tx.GetHash(),vout) == 0 && (numvouts=tx.vout.size())>0 && tx.vout[vout].nValue == CC_MARKER_VALUE && 
            DecodeGatewaysWithdrawSignOpRet(tx.vout[numvouts-1].scriptPubKey,tmpv,tmppk,withdrawtxid,tmplasttxid,signingpubkeys,coin,K,hex)=='S' && K>=M && refcoin==coin &&
            myGetTransaction(withdrawtxid,withdrawtx,hashBlock) != 0 && (numvouts= withdrawtx.vout.size())>0 &&
            DecodeGatewaysWithdrawOpRet(withdrawtx.vout[numvouts-1].scriptPubKey,tmpv,tmppk,tmptokenid,tmpbindtxid,coin,withdrawpub,amount) == 'W' && tmpbindtxid==bindtxid && tmptokenid==tokenid)
                txs.push_back(tx);
    }
    SetCCunspents(unspentOutputs,coinaddr,true);
    for (std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue> >::const_iterator it=unspentOutputs.begin(); it!=unspentOutputs.end(); it++)
    {
        ...
        vout = (int32_t)it->first.index;
        nValue = (int64_t)it->second.satoshis;
        if ( myIsutxo_spentinmempool(ignoretxid,ignorevin,txid,vout) == 0 && vout == 0 && nValue == CC_MARKER_VALUE && myGetTransaction(txid,tx,hashBlock) != 0 && (numvouts= tx.vout.size())>0 &&
            DecodeGatewaysWithdrawSignOpRet(tx.vout[numvouts-1].scriptPubKey,tmpv,tmppk,withdrawtxid,tmplasttxid,signingpubkeys,coin,K,hex)=='S' && K>=M && refcoin==coin &&
            myGetTransaction(withdrawtxid,withdrawtx,hashBlock) != 0 && (numvouts= withdrawtx.vout.size())>0 &&
            DecodeGatewaysWithdrawOpRet(withdrawtx.vout[numvouts-1].scriptPubKey,tmpv,tmppk,tmptokenid,tmpbindtxid,coin,withdrawpub,amount) == 'W' &&  tmpbindtxid==bindtxid && tmptokenid==tokenid)
                txs.push_back(tx);
    ...
    for (std::vector<CTransaction>::const_iterator it=txs.begin(); it!=txs.end(); it++)
    {
        tx = *it; 
        vout =0;
        if (myIsutxo_spentinmempool(ignoretxid,ignorevin,tx.GetHash(),vout) == 0 && (numvouts=tx.vout.size())>0 && DecodeGatewaysWithdrawSignOpRet(tx.vout[numvouts-1].scriptPubKey,tmpv,tmppk,withdrawtxid,tmplasttxid,signingpubkeys,coin,K,hex)=='S' && 
            K>=M && myGetTransaction(withdrawtxid,withdrawtx,hashBlock) != 0 && (numvouts= withdrawtx.vout.size())>0 && DecodeGatewaysWithdrawOpRet(withdrawtx.vout[numvouts-1].scriptPubKey,tmpv,tmppk,tmptokenid,tmpbindtxid,coin,withdrawpub,amount) == 'W')                   
        {
            UniValue obj(UniValue::VOBJ);
...
UniValue GatewaysList()
{
    ...
    CTransaction vintx; std::string coin; int64_t totalsupply; char str[65],depositaddr[64]; uint8_t version,M,N,taddr,prefix,prefix2,wiftype;
    std::vector<CPubKey> pubkeys; CPubKey tmppk;
    ...
    for (std::vector<uint256>::const_iterator it=txids.begin(); it!=txids.end(); it++)
    {
        ...
        if ( myGetTransaction(txid,vintx,hashBlock) != 0 )
        {
            if ( vintx.vout.size() > 0 && DecodeGatewaysBindOpRet(depositaddr,vintx.vout[vintx.vout.size()-1].scriptPubKey,version,tmppk,tokenid,coin,totalsupply,oracletxid,M,N,pubkeys,taddr,prefix,prefix2,wiftype) != 0 )
            {
...
UniValue GatewaysExternalAddress(uint256 bindtxid,CPubKey pubkey)
{
    ...
    std::string coin; int64_t numvouts,totalsupply; char str[65],addr[65],depositaddr[65]; uint8_t version,M,N,taddr,prefix,prefix2,wiftype;
    std::vector<CPubKey> msigpubkeys; CPubKey tmppk;
    ...
    if ( DecodeGatewaysBindOpRet(depositaddr,tx.vout[numvouts-1].scriptPubKey,version,tmppk,tokenid,coin,totalsupply,oracletxid,M,N,msigpubkeys,taddr,prefix,prefix2,wiftype) != 'B')
    {
        result.push_back(Pair("result","error"));
        result.push_back(Pair("error",strprintf("invalid bindtxid %s coin.%s",uint256_str(str,bindtxid),coin.c_str())));     
        return(result);
...
{
    UniValue result(UniValue::VOBJ); struct CCcontract_info *cp,C; uint256 txid,hashBlock,oracletxid,tokenid; CTransaction tx;
    std::string coin,priv; int64_t numvouts,totalsupply; char str[65],addr[65],depositaddr[65]; uint8_t version,M,N,taddr,prefix,prefix2,wiftype;
    std::vector<CPubKey> msigpubkeys; CPubKey tmppk;
...
{
    CMutableTransaction mtx = CreateNewContextualCMutableTransaction(Params().GetConsensus(), komodo_nextheight());
    UniValue result(UniValue::VOBJ),a(UniValue::VARR); std::string coin; char str[67],numstr[65],depositaddr[64];
    uint8_t M,N; std::vector<CPubKey> pubkeys; uint8_t version,tmpv,taddr,prefix,prefix2,wiftype; uint256 tokenid,oracletxid,hashBlock; CTransaction tx;
    CPubKey tmppk,gatewayspk; struct CCcontract_info *cp,C; int32_t i; int64_t numvouts,totalsupply,remaining; std::vector<CPubKey> msigpubkeys;
    ...
    if ( M <= N && N > 0 )
    {
        ...
        remaining = AddTokenCCInputs(cp,mtx,gatewayspk,tokenid,0,0);
